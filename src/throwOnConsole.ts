...
function throwError(message: string, overriddenConsoleMethod: ConsoleMethod, options: Options) {
  ...
  // By appending a zero-width space at each line of the "React stack trace" it forces Jest to ignore these lines
  //
  ...
  //    at :8080/form
  //
  // Prepending a zero-width space fixes this
  //
  // We also need a regular space before the zero-width space to make link "(http://0.0.0.0:8080/main.development.c190e20758c6a09d.js:5199:5)"
  // clickable again under Chrome
  //
  const msg = message
    .split('\n')
    .map(str =>
      str.replace(
        /^ {4}at .*$/,
        match => `${zeroWidthSpace}${match}${spaceToFixLinkChrome}${zeroWidthSpace}`
      )
    )
    .join('\n');
